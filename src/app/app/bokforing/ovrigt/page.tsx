...
export default function OvrigtPage() {
  ...
  if (done) {
    const typLabel = TYPER.find(t => t.id === typ)?.label ?? 'Övrigt';
    const riktningLabel = RIKTNINGAR.find(r => r.id === riktning)?.label ?? riktning;
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col">
        ...
        <div className="px-6 pb-12 max-w-xl mx-auto w-full flex flex-col gap-4">
          <div className="bg-white rounded-2xl border border-slate-200 p-5">
            ...
            <div className="grid grid-cols-2 gap-x-6 gap-y-3">
              <div>
                ...
                <p className="text-sm font-semibold text-slate-700">{typ === 'annat' ? beskrivning : typLabel}</p>
              </div>
              <div>
                <p className="text-xs text-slate-400 mb-0.5">Riktning</p>
                <p className="text-sm font-semibold text-slate-700">{riktningLabel}</p>
              </div>
              <div>
          ...
          <div className="flex gap-3">
            <button
              type="button"
              onClick={() => { setDone(false); setStep(0); setTyp(''); setBeskrivning(''); setRiktning(''); setBelopp(''); setDatum(TODAY); }}
