...
/**
 * Zettle-synken för alla kopplade kunder — för hand.
 *
...
export async function GET(request: Request) {
  ...
  try {
    return NextResponse.json(await synkaAllaZettle(createServerClient()));
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    console.error('[cron/zettle] körningen avbröts:', message);
