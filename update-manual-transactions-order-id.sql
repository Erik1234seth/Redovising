...
-- Step 2: Create a new column with UUID type
ALTER TABLE manual_transactions
ADD COLUMN IF NOT EXISTS order_uuid UUID;

-- Step 3: Update existing records to convert TEXT order_id to UUID
...
-- Step 5: Rename the new UUID column to order_id
ALTER TABLE manual_transactions
RENAME COLUMN order_uuid TO order_id;

-- Step 6: Add NOT NULL constraint (after ensuring all records have order_id)
...
-- Step 7: Add foreign key constraint to orders table
ALTER TABLE manual_transactions
...
-- Step 9: Recreate the view with the new structure
